...
# 设置工作目录
# -------------------------------------------------------------------
!cd "C:\Program Files\NSIS_xl\Resources"
...
!define SFX_Title "迅雷远征"; 安装程序的标题
!define SFX_InstallDir "$PROGRAMFILES\Thunder Network\迅雷远征"; *默认* 的目标安装目录
!define SFX_OutFile "H:\setup\xlyzwei_setup.exe"; 生成的安装程序文件
...
BrandingText "Copyright(C) 2012，迅雷看看"
Name "${SFX_Title}"
OutFile "${SFX_OutFile}"
...
# !insertmacro MUI_PAGE_LICENSE "C:\Program Files\NSIS_xl\license_xl.txt"
...
StrCpy $INSTDIR "$largestDisk迅雷看看\迅雷远征"
FunctionEnd
...
Section
  SetOutPath "$INSTDIR"
	...
	#删除一些文件夹先
	 ...
	 Delete /REBOOTOK $INSTDIR\data\yace\patch.mpk
	 RMDir /r /REBOOTOK $INSTDIR\bin 


    File /r "I:\迅雷绿色版\wei_setup\config.ini"
    File /r "I:\迅雷绿色版\wei_setup\updatescheme.xml"
    # File /r "I:\迅雷绿色版\wei_setup\yzonline_update.exe"
    File /r "I:\迅雷绿色版\wei_setup\远征Online.exe"
    # File /r "I:\迅雷绿色版\wei_setup\dbghelp.dll"
    File /r "I:\迅雷绿色版\wei_setup\serverlist.xml"
    # File /r "I:\迅雷绿色版\wei_setup\download"
    File /r "I:\迅雷绿色版\wei_setup\Skin"
    File /r "I:\迅雷绿色版\wei_setup\Bin"
    File /r "I:\迅雷绿色版\wei_setup\data"
...
Section
  ...
  CreateDirectory "$SMPROGRAMS\迅雷远征"
  CreateShortCut "$SMPROGRAMS\迅雷远征\迅雷远征Online.lnk" "$INSTDIR\远征Online.exe"
...
Section
  SetShellVarContext all
  CreateShortCut "$DESKTOP\迅雷远征Online.lnk" "$INSTDIR\远征Online.exe"
  CreateShortCut "$QUICKLAUNCH\迅雷远征Online.lnk" "$INSTDIR\远征Online.exe"
  SetShellVarContext current
SectionEnd
...
Section "Uninstall"
  SetOutPath $TEMP
  # nsExec::ExecToLog 'cmd /c copy "$INSTDIR\skin\jzinstall.exe" "$TEMP\jzinstall.exe"'
  # RMDir /r "$INSTDIR"
# 只干掉我们安装的文件和目录
  ...
  RMDir /r /REBOOTOK "$INSTDIR\UserData"
  Delete /REBOOTOK "$INSTDIR\config.ini"
  Delete /REBOOTOK "$INSTDIR\updatescheme.xml"
  Delete /REBOOTOK "$INSTDIR\远征Online.exe"
  Delete /REBOOTOK "$INSTDIR\serverlist.xml"
  Delete /REBOOTOK "$INSTDIR\updatelog.html"
  Delete /REBOOTOK "$INSTDIR\dns.ini"
  Delete /REBOOTOK "$INSTDIR\dbghelp.dll"
  Delete /REBOOTOK "$INSTDIR\远征Online.exe.tmp"
  Delete /REBOOTOK "$INSTDIR\Uninstall.exe"

  # !insertmacro MUI_STARTMENU_GETFOLDER Application $R0
  SetShellVarContext all
  # RMDir /r "$SMPROGRAMS\$R0"
  RMDir /r "$SMPROGRAMS\迅雷远征"
  SetShellVarContext current

  SetShellVarContext all
  Delete "$DESKTOP\迅雷远征Online.lnk"
  Delete "$QUICKLAUNCH\迅雷远征Online.lnk"
  SetShellVarContext current
...
Function un.onUninstSuccess
  ...
  # ExecShell "open" "http://api.yz.q1.com/GameInstall/GameUnInstall.aspx?$EnterDescription&remark=xunleiyz_wei&pid=9"
  # ExecShell "open" "$TEMP\jzinstall.exe"
  # MessageBox MB_OK|MB_ICONSTOP|MB_TOPMOST "http://api.yz.q1.com/GameInstall/GameUnInstall.aspx$EnterDescription"
FunctionEnd
...
# MessageBox MB_OK "Mac=$MacAddress"  
  # ExecShell open "$PROGRAMFILES\Internet Explorer\iexplore.exe" "http://api.yz.q1.com/GameInstall/Install.aspx?GameName=yz3&Mac=$MacAddress&Remark=xunleiyz_wei&pid=9" SW_SHOWMINIMIZED
