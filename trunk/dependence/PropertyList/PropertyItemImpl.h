...
public:
   CProperty(LPCTSTR pstrName, LPARAM lParam) : m_fEnabled(true), m_lParam(lParam), m_hWndOwner(NULL), m_bVisible(true), m_bNumberOnly(false)
   {
...
//	  rcItem.bottom += 10;
   ...
   virtual void setNumberOnly(bool b)
   {
	   m_bNumberOnly = b;
   }
...
//	SubscriberSlot mSlot;
public:
   CPropertyFileNameItem(LPCTSTR pstrName, LPARAM lParam) : CPropertyItem(pstrName, lParam),mFileDlg(TRUE)
   {
	   
   }

   CPropertyFileNameItem(LPCTSTR pstrName, LPCTSTR pstrFilename, LPARAM lParam) : CPropertyItem(pstrName, lParam),mFileDlg(TRUE)
   {
	   m_val = pstrFilename;
   }
//    void setSlot(SubscriberSlot slot)
//    {
// 	   mSlot = slot;
//    }
   void doModal()
   {
	   ...
	   {
		   //fire event, the modle file has been changed
		   /*
		   EventArgs_ModelFileChanged args;
		   args.mPropertyFileNameItem = this;
		   args.mFileName = mFileDlg.m_szFileName;
		   args.mFileNameShort = mFileDlg.m_ofn.lpstrFile;
		   EventManager::GetInstance().fireEvent(eEvent_ModelFileChanged, &args);
		   */
// 		   EventArgs_ModelFileItemDoModal args;
// 		   args.mPropertyFileNameItem = this;
// 		   mSlot(args);
	   }
   }
...
inline HPROPERTY PropCreateSimpleString(LPCTSTR pstrName, LPCTSTR pstrValue, LPARAM lParam = 0)
{
   return new CPropertyEditItem(pstrName, CComVariant(pstrValue), lParam);
}

inline HPROPERTY PropCreateSimpleInt(LPCTSTR pstrName, int iValue, LPARAM lParam = 0)
{
   return new CPropertyEditItem(pstrName, CComVariant(iValue), lParam);
}

inline HPROPERTY PropCreateSimpleFloat(LPCTSTR pstrName, float fValue, LPARAM lParam = 0)
{
	return new CPropertyEditItem(pstrName, CComVariant(fValue), lParam);
}

inline HPROPERTY PropCreateSimpleBool(LPCTSTR pstrName, bool bValue, LPARAM lParam = 0)
{
