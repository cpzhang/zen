...
public:
   ...
   //add prop after head
   HPROPERTY AddItem(HPROPERTY prop, HPROPERTY head, CCategoryProperty* ct /*= NULL*/)
   {
	   ATLASSERT(::IsWindow(m_hWnd));
	   ATLASSERT(prop);
	   if( prop == NULL ) return NULL;
	   prop->SetOwner(m_hWnd, NULL);
	   int nItem = FindProperty(head);
	   ++nItem;
	   TBase::InsertString(nItem, prop->GetName());
   ...
   HPROPERTY AddItem(HPROPERTY prop, CCategoryProperty* ct /*= NULL*/)
   {
      ATLASSERT(::IsWindow(m_hWnd));
      ATLASSERT(prop);
      if( prop == NULL ) return NULL;
   ...
   {      
      ...
      RECT rc = { 0 };
      GetClientRect(&rc);
      if( !m_bColumnFixed ) m_iMiddle = (rc.right - rc.left) / 6;
      m_iPrevious = 0;
      BOOL bDummy = FALSE;
   ...
   LRESULT OnSettingChange(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
   {
	  ...
	  std::string fn("宋体");
	  strncpy(lf.lfFaceName, fn.c_str(), 32);
...
// 	  }
      ...
      if( (m_dwExtStyle & PLS_EX_CATEGORIZED) != 0 ) {
         ...
         if( kind == PROPKIND_CATEGORY )
		 {
            ...
            CCategoryProperty* pCategory = static_cast<CCategoryProperty*>(prop);
			rc.left += CATEGORY_INDENT * pCategory->GetLevel();
		 ...
		 {
			 CCategoryProperty* pCategory = prop->getCategory();
			 if (pCategory)
			 {
				 rc.left += CATEGORY_INDENT * pCategory->GetLevel();
			 }
      ...
      if( !(kind == PROPKIND_CATEGORY && (m_dwExtStyle & PLS_EX_XPLOOK) != 0) && prop->hasMiddleLine()) {
